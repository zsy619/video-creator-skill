...
- [references/audio-voice-identification.md](references/audio-voice-identification.md) — **音频声音识别**：从 neural_1_2x.m4a 文件名/格式推断 edge-tts 语音，无需项目日志；含 filename pattern → voice type 对照表
...
- [references/dynamic-scene-boundaries.md](references/dynamic-scene-boundaries.md) — 动态场景帧边界计算
- [references/scene-visual-upgrade-pattern.md](references/scene-visual-upgrade-pattern.md) — **6场景视觉增强模式**：粒子/霓虹/glow/终端窗口/glitch等系统化重绘方案（2026-05-15实测）：取代硬编码 `SCENE_BOUNDARIES`，基于 `sceneFractions` 比例自动分配，支持任意场景数量
...
- [references/edge-tts-service-unavailable-2026-05-15.md](references/edge-tts-service-unavailable-2026-05-15.md) - **edge-tts 服务不可用**：Microsoft 语音合成服务阻断，`zh-CN-YunJiaNeural` 失败，`zh-CN-XiaoxiaoNeural` 备用方案
