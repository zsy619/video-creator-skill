...
| 文件 | 用途 |
|------|------|
| `feishu-base-batch-problems.md` | Feishu Base 批量处理失败项目列表、根因分析、atempo规则 |
...
> ⚠️ **深度知识库**：[references/feishu-base-batch-problems.md](references/feishu-base-batch-problems.md) 包含完整问题列表、11个受影响项目、根因分析、atempo 动态计算规则、音频命名规范、封面图规范。
...
> 深度说明见 [references/feishu-base-batch-problems.md](references/feishu-base-batch-problems.md#4-atempo-动态计算规则)
...
**Step 0 后强制检查**：
- 执行后立即统计 narration.txt 中文字符数（Python `\u4e00-\u9fff`）
- 触发重写条件（满足任一）：中文字数 < 20 / 含 `---`、`|` 等 markdown 噪声 / 字数 > 上限 2 倍（350 字）
...
> ⚠️ **完整列表**：详见 [references/CHANGELOG.md](references/CHANGELOG.md)（含18个主文件 + archived/ 归档说明）

| 类别 | 核心文档 |
|------|----------|
| **核心流程** | [content-document-generation.md](references/content-document-generation.md) · [subagent-timeout.md](references/subagent-timeout.md) · [CHANGELOG.md](references/CHANGELOG.md) |
...
| **文档Git** | [documentation-consistency.md](references/documentation-consistency.md) · [feishu-base-api.md](references/feishu-base-api.md) · [git-workflow.md](references/git-workflow.md) · [openhuman-narration.md](references/openhuman-narration.md) |
...
**核心约束**：配音文本字数上限 = ⌊目标时长 × 3.37⌋（实测 3.73 字/秒，留 10% 余量）。禁止分段拼接；禁止跳过音频后处理（去静音 + 1.2x + AAC 256k）；禁止 edge-tts rate=+20% + atempo=1.2x 叠加。
