...
> **默认男声**：`zh-CN-YunyangNeural`（新闻男声，稳重）
> 这是用户的永久偏好，不接受女声配音。所有视频项目默认使用此语音。

**语音优先级**：
1. `zh-CN-YunyangNeural` — **默认男声**（用户偏好）
2. `zh-CN-YunjianNeural` — 温和男声（备选）
3. `zh-CN-YunxiNeural` — 年轻男声（科技/工具类）

**Fallback 规则（重要）**：当首选语音不可用时，**必须选择另一个男声**，禁止降级到女声：
- ❌ 错误：`zh-CN-YunyangNeural` 不可用 → 降级到 `zh-CN-XiaoxiaoNeural`（女声）
- ✅ 正确：`zh-CN-YunyangNeural` 不可用 → 尝试 `zh-CN-YunjianNeural` 或 `zh-CN-YunxiNeural`
