...
[![Version](https://img.shields.io/badge/version-1.2.0-green)](https://github.com/zsy619/video-creator-skill)
...
### 🎯 12步工作流
1. **Step 0: 创建文档** - 初始化全部 11 个文档文件 ⭐ **新增**
2. **获取内容** - 从URL/主题/文件/直接内容获取原始素材
3. **分析内容** - 提取关键词、计算时长、生成摘要
4. **构建项目** - 创建标准化目录结构
5. **生成文案** - 平台优化标题、标签、摘要
6. **构建HTML** - Tailwind CSS 响应式页面
7. **生成视觉** - 封面图、插图、信息图（封面强制优先）
8. **生成音频** - edge-tts自然人声配音
9. **生成字幕** - 智能生成ASS字幕，自动修复字体兼容性
10. **质量检查** - 检查字体、音频、字幕、视频质量
11. **生成视频** - Remotion渲染60fps竖屏视频
12. **生成报告** - 完整的执行报告

### 🔄 执行模式
- **完整模式**: 运行 Step 0-11 全流程
- **检查模式**: 只运行质量检查（步骤9）
- **字幕模式**: 只生成字幕（步骤8）
- **批量模式**: 批量处理多个项目
