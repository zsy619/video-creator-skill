...
**问题**：长句子如果不换行，会超出屏幕边界造成遮挡。

**解决方案**：使用 `\N` 换行符将长字幕智能分段为多行显示。

**换行原则**：
1. 每行不超过 **15-20个字符**（中文约7-10个字）
2. 按**语义停顿点**换行，不要在单词中间断开
3. 保持每行长度相近，避免出现"头重脚轻"
4. 最多 **3 行**，超过3行需要拆分成多个字幕片段

**示例**：
```ass
# ❌ 错误：单行太长，超出屏幕
Dialogue: 0,0:00:03.50,0:00:07.50,Default,,30,30,30,,今天给大家介绍一个开源工具 Audiblez，它可以把你书架上的电子书一键变成有声书。

# ✅ 正确：智能分段，多行显示
Dialogue: 0,0:00:03.50,0:00:07.50,Default,,30,30,30,,今天给大家介绍一个开源工具\NAudiblez，它可以把你书架上的电子书\N一键变成有声书。
```

**ASS文件头必须包含 `WrapStyle: 0`**：
```ass
[Script Info]
Title: Video Creator Subtitles
ScriptType: v4.00+
...
| 原始文本 | 分段结果 |
|---------|---------|
| 今天给大家介绍一个开源工具 Audiblez | 今天给大家介绍一个开源工具\NAudiblez |
| 它使用 Kokoro-82M 语音合成模型。82M 参数，却能产生极其自然的语音。 | 它使用 Kokoro-82M 语音合成模型。\N82M 参数，却能产生极其自然的语音。 |
| 支持9种语言，包括英语、中文、日语、西班牙语等等。 | 支持9种语言，包括英语、中文、\N日语、西班牙语等等。 |
