...
> ⚠️ **字号为固定值，不再使用比例计算**。所有文字必须通过 `textbbox` 自校验宽度（<90%画布宽）。

| 封面类型 | 画布尺寸 | 主标题 | 副标题1 | 副标题2 | 标签 |
|---------|---------|--------|--------|--------|------|
| 竖屏 | 1080×1920 | **130px** | **52px** | **36px** | 32px |
| 公众号 | 900×383 | **108px** | **44px** | — | 28px |
| 小红书 | 1440×2560 | **180px** | **72px** | **48px** | 48px |
...
> ⚠️ **所有 PIL 封面生成必须包含文字宽度校验**，防止文字溢出画布。

```python
def measure_text(draw, text, font, anchor='mm'):
    """测量文字边界，返回 (width, height)"""
    bbox = draw.textbbox((0, 0), text, font=font, anchor=anchor)
    return bbox[2] - bbox[0], bbox[3] - bbox[1]

def smart_resize_text(text, font_path, start_size, max_width_ratio=0.90):
    """
    自动缩减字号直到文字宽度符合要求
    size: 起始字号
    max_width_ratio: 最大宽度占比（默认90%）
...
assert tw < 1080 * 0.90, f"文字仍超出画布: {tw}px"
```

### 固定字号表（已验证安全）

| 画布宽度 | 主标题字号 | 副标题1 | 副标题2 |
|---------|-----------|--------|--------|
| 1080 | 130px | 52px | 36px |
| 900 | 108px | 44px | — |
| 1440 | 180px | 72px | 48px |

> ✅ 以上字号已通过实测验证，12字符内中文副标题均 < 90% 画布宽。
...
| 元素 | 字体大小 | 说明 |
| ---- | -------- | ---- |
| 主标题 | **130px** | 封面场景（英文≤12字符不换行） |
| 副标题 | **52px** | 封面场景中文说明 |
| 内容标题 | **72-96px** | 功能/章节标题 |
| 内容正文 | **36-48px** | 描述文字 |
| 命令行 | **22-28px** | 代码/命令展示 |
